...
struct urma_eid_t;
struct urma_eid_info_t;
struct urma_device_attr_t;
...
// URMA Device abstraction (similar to ib_device_t)
class urma_device_t {
 public:
  urma_device_t();
  ~urma_device_t();

  urma_device_t(const urma_device_t&) = delete;
  urma_device_t& operator=(const urma_device_t&) = delete;

  bool init(const std::string& device_name, int eid_index = 0);
  void close();

  urma_context_t* context() const { return context_; }
  urma_device_t* device() const { return device_; }
  ...
  bool is_valid() const { return context_ != nullptr && device_ != nullptr; }
  std::shared_ptr<urma_buffer_pool_t> get_buffer_pool() const { return buffer_pool_; }

 private:
  std::string name_;
  int eid_index_ = -1;
  urma_device_t* device_ = nullptr;
...
class urma_device_manager {
 public:
  ...
  bool init();
  std::shared_ptr<urma_device_t> get_device(const std::string& device_name = "");
  std::vector<std::shared_ptr<urma_device_t>> get_all_devices();
  std::shared_ptr<urma_device_t> get_global_device();

 private:
  urma_device_manager() = default;
  ~urma_device_manager();
  bool initialized_ = false;
  std::vector<std::shared_ptr<urma_device_t>> devices_;
  std::shared_ptr<urma_device_t> global_device_;
};

inline std::shared_ptr<urma_device_t> get_global_urma_device() {
  return urma_device_manager::instance().get_global_device();
}

// ============= Implementation (inline in header) =============

inline urma_device_t::urma_device_t() = default;

inline urma_device_t::~urma_device_t() { close(); }

inline bool urma_device_t::init(const std::string& device_name, int eid_index) {
#ifdef YLT_ENABLE_URMA
  name_ = device_name;
  ...
  device_ = found_device;

  uint32_t eid_cnt = 0;
  urma_eid_info_t* eid_list = urma_get_eid_list(device_, &eid_cnt);
  ...
  context_ = urma_create_context(device_, eid_index_);
  if (!context_) {
  ...
  if (urma_query_device(device_, &device_attr_) != 0) {
...
inline void urma_device_t::close() {
#ifdef YLT_ENABLE_URMA
  if (context_) {
    urma_delete_context(context_);
    context_ = nullptr;
...
inline std::string urma_device_t::eid_string() const {
  char buf[64] = {0};
...
inline asio::ip::address urma_device_t::gid_address() const {
  char buf[64];
...
inline std::shared_ptr<urma_device_t> urma_device_manager::get_device(
  ...
  auto dev = std::make_shared<urma_device_t>();
...
inline std::vector<std::shared_ptr<urma_device_t>>
urma_device_manager::get_all_devices() {
  ...
  int num_devices = 0;
  urma_device_t** urma_devices = urma_get_device_list(&num_devices);
  if (!urma_devices || num_devices <= 0) return devices_;

  for (int i = 0; i < num_devices; ++i) {
    auto dev = std::make_shared<urma_device_t>();
    if (dev->init(urma_devices[i]->name)) {
      devices_.push_back(dev);
      if (!global_device_) global_device_ = dev;
    }
  }
  urma_free_device_list(urma_devices);
  return devices_;
#else
  return {};
#endif
...
inline std::shared_ptr<urma_device_t> urma_device_manager::get_global_device() {
